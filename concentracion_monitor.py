        ...
        if temp_data:
            ...
            for zona_precio in top_volumen.index:
                if abs(precio_actual - zona_precio) < (precio_actual * 0.004):
                    enviar = False
                    # Redondeamos para que pequeñas variaciones no disparen alertas falsas
                    zona_key = str(round(float(zona_precio), 1)) 
                    
                    if tf not in memoria_alertas:
                        enviar = True
                    else:
                        ultima_zona = memoria_alertas[tf].get('zona')
                        ultima_fecha = memoria_alertas[tf].get('fecha')
                        
                        # CRITERIO: Punto distinto O día distinto
                        if zona_key != str(ultima_zona) or fecha_hoy != ultima_fecha:
                            enviar = True
                    
                    if enviar:
                        memoria_alertas[tf] = {'zona': zona_key, 'fecha': fecha_hoy}
                        guardar_memoria(memoria_alertas) # Persistencia inmediata
                    ...
                    break # Solo procesamos la zona más relevante detectada
